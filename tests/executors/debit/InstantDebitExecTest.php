...
class InstantDebitExecTest extends TestCase
{
    use HttpMockTrait;
    ...
    public function testInstantDebit() {
        ...
        $this->http->mock
            ->when()
            ->methodIs('POST')
            ->pathIs('/v1/debit/instant')
            ->then()
            ->body($mockResponse)
            ->end();
        $this->http->setUp();

        $testRequest = new InstantDebitRequest();
        ...
        $result = $client->instantDebit($testRequest);
