...
class RegularDebitExecTest extends TestCase
{
    use HttpMockTrait;
    ...
    public function testRegularDebit() {
        $mockResponse = '{"success":true,"code":"SUCCESS","redirectUrl":"https://mercury-uat.phonepe.com/v1/debit/action?token=ABCXYZTOKEN","message":"Please Redirect to the given url.","data":null}';
        $this->http->mock
            ->when()
                ->methodIs('POST')
                ->pathIs('/v1/debit/')
        ...
        $testRequest = new RegularDebitRequest();
        ...
        $result = $client->regularDebit($testRequest);

        $this->assertEquals($result->success, true);
        $this->assertEquals($result->code, "SUCCESS");
        $this->assertEquals($result->redirectUrl, "https://mercury-uat.phonepe.com/v1/debit/action?token=ABCXYZTOKEN");
    }
}