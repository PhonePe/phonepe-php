...
class DeferredCreditTest extends TestCase
{
    use HttpMockTrait;
    ...
    public function testDeferredCredit() {
        $mockResponse = '{"success":true,"code":"SUCCESS","redirectUrl":"https://mercury-uat.phonepe.com/v1/credit/deferred/action?token=ABCXYZTOKEN","message":"Please Redirect to the given url.","data":null}';
        $this->http->mock
            ->when()
                ->methodIs('POST')
                ->pathIs('/v1/credit/deferred')
            ->then()
                ->body($mockResponse)
            ->end();
        $this->http->setUp();

        $testRequest = new DeferredCreditRequest();
        ...
        $result = $client->deferredCredit($testRequest);

        $this->assertEquals($result->success, true);
        $this->assertEquals($result->code, "SUCCESS");
        $this->assertEquals($result->redirectUrl, "https://mercury-uat.phonepe.com/v1/credit/deferred/action?token=ABCXYZTOKEN");
    }
}